...
int main (void) {
    ...
    if (count > 0) {
        ...
        char *s3 = "        char *s1 = %2$c%3$s%2$c;%1$c        char *s2 = %2$c%4$s%2$c;%1$c        char *s3 = %2$c%5$s%2$c;%1$c        fprintf(fd, s1, 10);%1$c        fprintf(fd, s2, 10, count, 34);%1$c        fprintf(fd, s3, 10, 34, s1, s2, s3);%1$c        fclose(fd);%1$c        count != 0 ? system(compile): 0;%1$c    }%1$c    return 0;%1$c}";
        fprintf(fd, s1, 10);
        fprintf(fd, s2, 10, count, 34);
        fprintf(fd, s3, 10, 34, s1, s2, s3);
        fclose(fd);
        count != 0 ? system(compile): 0;
    }
    return 0;
}